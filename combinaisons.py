...
def teste():
    ...
    recupere_combinaisons([0, 1, 2, 3, 4], 3)
...
def recupere_combinaisons_2(sequence):
    ...
    taille = len(sequence)
    list_comb = []
    for deb in range (0, taille):
        for fin in range(deb, taille):
            list_comb.append((list_comb[deb], list_comb[fin]))
...
def recupere_combinaisons(sequence, k):
    ...
    return subcombination(sequence, [], k, 0)

def subcombination(seq, accu, nbr_rest, idx):
    """retourne la liste des listes de concaténation de accu 
        avec toutes les combinaisons de de la sequence de nbr_rest
        éléments partants de idx
        prérequis : nbr_rest <=  len(seq)-idx """

    print(f"appel sucombination({seq =}, {accu =},{nbr_rest =}, {idx =})")
    ...
    list_l = []
    length = len(seq)
    for i in range(idx, length-nbr_rest+1):
        for liste in subcombination(seq, accu+[seq[i]], nbr_rest-1, i+1):
            print(f"{list_l =}")
            list_l.append(liste)
...
if __name__ == "__main__":
    l = recupere_combinaisons([1, 2, 3, 4, 5], 3)
    print(f"{l =}")